function [graz_ang, geo_ang_as, x_spec, y_spec, x_trans, y_trans, u] = get_reflection_spherical_fujimura (e, Ha, Ht, Rs)
...
posgeo2complex = @(pos) complex(pos(1), pos(2))./Rs;
complex2posgeo = @(cpx) [real(cpx), imag(cpx)].*Rs;  % in geo frame
...
z1 = posgeo2complex(pos_ant_geo);  % as complex number #CORRECT

% Transmitting satellite position:
[pos_trans_loc, pos_trans_geo] = get_satellite_position (e, Ha, Ht, Rs, 2);
z2 = posgeo2complex(pos_trans_geo); % #CORRECT
...
x_spec = pos_spec_loc(1); % #CORRECT
y_spec = pos_spec_loc(2); % #CORRECT
x_trans = pos_trans_loc(1);
y_trans = pos_trans_loc(2);

%% Geocentric angle between antenna and reflection point
geo_ang_as = 90-rad2deg(angle(u));

%% Grazing Angle

% graz_ang = atand(cotd(geo_ang)-(Rs./Ra)./sind(geo_ang)); #OLD 
graz_ang = 90-rad2deg(angle(z1/u-1));
